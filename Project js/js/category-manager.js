...
const productPageButton = document.getElementById("btn-product-page");
...
const validateVisible = (indices, isVisible) => {
  ...
  indexArray.forEach((idx) => {
    ...
    const inputElement = validateText.parentElement.querySelector("input"); //Lấy phần tử cha và truy cập vào input của validate đó
...
//input: input category_input_code(giá input mã của danh mục), nhằm lấy giá trị để kiểm tra xem có trùng không
//input: category_input_name(giá trị input của tên danh mục ), nhằm lấy tên để kiểm tra xem có rỗng không
//output: giá trị đúng/sai, chỉ cần 1 điều kiện ko đáp ứng sẽ trả về sai
const validateInput = (category_input_code, category_input_name) => {
  let isContinue = true; //giá trị cuối cùng được trả về

  //category validate
  const categoryValue = category_input_code.trim();
  const foundIndex = getCategoryDataIndexByCategoryCode(categoryValue);
  ...
  const nameValue = category_input_name.trim();
...
const getFilteredData = (filter = "all", sortBy = "none", keyword = "") => {
  let dataWrapper = [];

  for (let i = 0; i < categoriesData[currUserData.id].length; i++) {
    //kiểm tra filter và từ khóa trước
    const element = categoriesData[currUserData.id][i];
    if (!element) continue; //nếu rỗng thì skip

    const name = element.category_name.toLowerCase();
    const code = element.category_code.toLowerCase();
...
//Lấy index dữ liệu của một danh mục từ mã danh mục(đầu vào mã danh mục)
const getCategoryDataIndexByCategoryCode = (category_code) => {
  for (let i = 0; i < categoriesData[currUserData.id].length; i++) {
    const element = categoriesData[currUserData.id][i];
    if (element.category_code == category_code) {
      return i;
    }
  }
  return -1;
...
const getCategoryDataIndexById = (id) => {
  for (let i = 0; i < categoriesData[currUserData.id].length; i++) {
    const element = categoriesData[currUserData.id][i];
    if (!element) {
      continue;
...
const dataToHTML = (data) => {
  return `
      <tr class="category-element" sp-data-id="${data.id}">
        <td class="category-code">${data.category_code}</td>
        <td class="font-medium category-name">${data.category_name}</td>
        <td class="status-display">
          <span class="status-badge status-${data.status == "ACTIVE" ? "active" : "inactive"}">
            <span class="dot"></span> ${data.status == "ACTIVE" ? "Đang" : "Không"} hoạt động
          </span>
        </td>
        <td class="actions">
...
const startupButtonTable = (elementPerPage = 7) => {
  const totalPage = Math.ceil(
    //lấy tổng trang
    categoriesData[currUserData.id].length / elementPerPage,
  );
  //lấy DOM đầu và cuối
...
const pageButtonCreate = (
  ...
  const totalPage = //tổng page
    fullPage ||
    Math.ceil(categoriesData[currUserData.id].length / elementPerPage);
...
const switchModalMode = (mode, data) => {
  ...
  const statusRadios = modalFrame.querySelectorAll(".status-radio");
  ...
  if (mode === "add") {
    ...
    if (modalTitle) modalTitle.textContent = "Thêm mới danh mục";
    if (saveButton) saveButton.textContent = "Thêm";
    if (inputs.length >= 2) {
      inputs[0].value = "";
      inputs[1].value = "";
    }
    // Mặc định chọn "Đang hoạt động" khi thêm mới
    ...
    if (modalTitle) modalTitle.textContent = "Cập nhật danh mục";
    if (saveButton) saveButton.textContent = "Lưu";
    if (inputs.length >= 2 && data) {
...
const handleChangeView = ({
  ...
  sortBy = currSortMode || "none",
  keyword = currSearchKeyWord,
} = {}) => {
  //kiểm tra sự thay đổi của filter và cập nhật trang được truyền vào, nếu thảy đổi đưa về page 1
  ...
  if (isFilterChange && filter != false) {
    //nếu filter không rỗng
    currFilter = filter;
  }

  if (isSortByChange && sortBy != false) {
    currSortMode = sortBy;
  }
  ...
  if (isFilterChange || isSortByChange || isKeywordChange) {
    //nếu filter hoặc sort đã thay đổi
    currPage = 1;
  }

  //lấy tổng element đang có filter và tính tổng
  ...
  //gọi hàm render các danh mục và nút bấm
  pageButtonCreate(
    currPage,
    2,
    elePerPage,
...
render(currPage, elePerPage, currFilter, currSortMode); //render mặc định
...
addCheckedToast("Xin chào", "bạn đang ở quản lý danh mục");
...
productDataTable.addEventListener("click", async (event) => {
  ...
  if (button.classList.contains("btn-action-edit")) {
    ...
    const categoryCode = row.querySelector(".category-code").textContent.trim(); //lấy mã danh mục
    const categoryName = row.querySelector(".font-medium").textContent.trim(); //lấy tên danh mục

    // Di chuyển hàm lấy index lên trước để lấy được status từ data gốc
    currEditIndex = getCategoryDataIndexById(
      Number(row.getAttribute("sp-data-id")),
    );

    // Lấy status từ data của user
    const currentStatus = categoriesData[currUserData.id][currEditIndex].status;

    // Truyền thêm status vào object data
    ...
    const dataIndex = getCategoryDataIndexById(
      Number(row.getAttribute("sp-data-id")),
    );
    ...
    if (
      await confirmPopUp(
        "Xác nhận",
        `Bạn có muốn xóa sản phẩm với mã <strong>${categoriesData[currUserData.id][dataIndex].category_code}<strong/>`,
        "Xóa",
      )
    ) {
      categoriesData[currUserData.id].splice(dataIndex, 1);
      saveData();
      handleChangeView({ page: currPage });
      addCheckedToast("Thành công", "Đã xóa danh mục thành công");
    }
  }
});
...
modalFrame.addEventListener("click", (event) => {
  ...
  if (button && button.classList.contains("btn-save")) {
    ...
    //Lấy input
    const category_input = modalFrame.getElementsByClassName("input-control");
    const status_input = modalFrame.getElementsByClassName("status-radio")[0];
    const category_input_code = category_input[0]; //input code category
    const category_input_name = category_input[1]; //input name category
    //sử dụng hàm validate input trước đó để quyết định xem có lưu lại và ẩn/hiện validate hay không
    if (!validateInput(category_input_code.value, category_input_name.value)) {
      return;
    }

    if (modalFrame.classList.contains("edit-mode")) {
      //sử dụng currEdit và currEditElement để điền
      categoriesData[currUserData.id][currEditIndex].category_code =
        category_input_code.value;
      categoriesData[currUserData.id][currEditIndex].category_name =
        category_input_name.value;
      currEditElement.querySelector(".category-code").textContent =
        category_input_code.value;
      currEditElement.querySelector(".category-name").textContent =
        category_input_name.value;
      ...
      if (status_input.checked) {
        categoriesData[currUserData.id][currEditIndex].status = "ACTIVE";
      } else {
        categoriesData[currUserData.id][currEditIndex].status = "INACTIVE";
      }
      ...
      const newCategoryData = {
        id:
          getHightestID(
            "categoriesDataHighestData",
            categoriesData[currUserData.id],
          ) + 1,
        category_name: category_input_name.value,
        category_code: category_input_code.value,
        status: `${status_input.checked ? "ACTIVE" : "INACTIVE"}`,
        created_at: getCurrentTime(),
      };
      categoriesData[currUserData.id].unshift(newCategoryData); //thêm vào dữ liệu danh mục hiện tại của người dùng
...
pageButtonTable.addEventListener("click", (event) => {
  ...
  if (button.classList.contains("page-btn")) {
    ...
    handleChangeView({ page: page, filter: currFilter, sortBy: currSortMode });
...
productPageButton.addEventListener("click", (event) => {
  switchTab("./product-manager.html");
});
...
// categoriesData[currUserData.id] = []; // reset để test
...
//         categoriesData[currUserData.id],
//       ) + 1,
//     category_name: "SP" + Math.floor(Math.random() * 1000), // tên random
//     category_code: "SP" + Math.floor(Math.random() * 1000),
//     status: Math.random() < 0.5 ? "ACTIVE" : "INACTIVE",
//     created_at: getRandomDate(new Date(2023, 0, 1), new Date()), // date random từ 1/1/2023 đến hiện tại
//   };
//   categoriesData[currUserData.id].push(newCategoryData);
// }

// saveData();
// render(currPage, elePerPage, currFilter, currSortMode);
